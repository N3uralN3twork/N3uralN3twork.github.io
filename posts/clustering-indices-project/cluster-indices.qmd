...
$n_{k}$ = the number of observations belonging to cluster \$\$C\_{k}\$\$

$x_{i}$ = p-dim vector of observations of the $i^{th}$ object in cluster $C_{k}$

$W_{q}$ = $\Large \sum^{K}_{k=1} \sum (x_{i} - c_{k}) (x_{i} - c_{k})^{T}$ is the within-group dispersion matrix for data clustered into $K$ clusters

$B_{q}$ = $\Large \sum^{K}_{k=1} n_{k} (c_{k} - \bar{x}) (c_{k} - \bar{x})^{T}$ is the between-group dispersion matrix for data clustered into $K$ clusters

$N_{t}$ = $\frac {n(n-1)}{2}$ is the total number of pairs of observations in the dataset

$N_{w}$ = $\Large \sum_{k=1}^{K} \frac {n_{k} (n_{k} - 1)}{2}$ is the total number of pairs of observations belonging to the same cluster

$N_{b}$ = $N_{t} - N_{w}$ is the total number of pairs of observations belonging to different cluster
